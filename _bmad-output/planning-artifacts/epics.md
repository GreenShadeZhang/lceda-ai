...
- **认证流程**：Keycloak OIDC Authorization Code + PKCE，token 在 IFrame 内 `callback.html` 换取后通过 postMessage 传主线程
- **API 风格**：REST + SSE（流式 LLM 输出），统一 `ApiResponse<T>` Wrapper
- **ORM**：EF Core + Npgsql，Code-First 迁移，`UseSnakeCaseNamingConvention()`
- **postMessage 协议**：6 种消息类型常量集中在 `messageTypes.ts`
...
### Epic 2: 用户认证 — Keycloak OIDC 登录

**目标：** 用户可在立创 EDA 插件 IFrame 内完成 Keycloak 登录，获取 JWT token 并安全存储，后端 API 能验证 token。
...
用户可在立创 EDA 插件 IFrame 内完成 Keycloak OIDC PKCE 登录，获取 JWT token 并安全存储到 `eda.sys_Storage`，后端 API 能通过 JWKS 验证 token 有效性，受保护接口正常工作。
...
**When** 点击"登录"按钮
**Then** IFrame 导航到 Keycloak 授权页面，URL 包含 `response_type=code`、`code_challenge`（PKCE）参数

**Given** 用户在 Keycloak 页面完成账号密码登录
**When** Keycloak 重定向到 `callback.html`
**Then** `callback.html` 使用授权码 + code_verifier 向 Keycloak token endpoint 换取 access_token 和 refresh_token，换取成功

**Given** `callback.html` 成功换取 token
**When** token 换取完成
**Then** 通过 `postMessage` 向主页面发送 `AUTH_SUCCESS` 消息，消息 payload 包含 `accessToken` 和 `refreshToken`

**Given** 登录过程中发生错误（如用户取消、密码错误）
**When** 错误发生
**Then** 通过 `postMessage` 发送 `AUTH_FAILURE` 消息，IFrame 显示错误提示
...
**Given** IFrame 通过 postMessage 发送 `AUTH_SUCCESS` 消息
**When** `index.ts` 主线程收到消息
**Then** 使用 `eda.sys_Storage.setItem()` 将 `accessToken` 和 `refreshToken` 存入 EDA 安全存储，不使用 `localStorage`

**Given** Token 已存储
**When** 用户关闭并重新打开插件面板
**Then** 插件从 `eda.sys_Storage` 读取 token，若有效则直接进入已登录状态，无需重新登录

**Given** Token 即将过期（access_token 剩余 < 60 秒）
**When** 用户发起操作
**Then** IFrame `auth.js` 使用 refresh_token 静默刷新，将新 token 通过 `AUTH_TOKEN_SYNC` 消息同步到主线程更新存储

**Given** refresh_token 也已过期
**When** 刷新失败
**Then** 清除存储的 token，IFrame 展示登录页面，`sys_ToastMessage` 提示"登录已过期，请重新登录"
...
**Then** IFrame 通过 `postMessage` 向主线程发送 `GENERATE_REQUEST` 消息，payload 包含完整的 `circuitJson`
...
**Given** `index.ts` 收到 `GENERATE_REQUEST` postMessage，payload 包含合法 `circuitJson`
**When** 调用 `componentValidator.ts`
...
**Given** 所有放置操作完成
**When** 调用保存
**Then** 执行 `eda.sch_Document.save()`，文档成功保存，`sys_ToastMessage.showMessage("原理图生成成功")` 展示

**Given** 任意元件放置失败（SDK 抛出异常）
**When** 异常捕获
**Then** 停止后续放置，通过 `sys_ToastMessage.showMessage("放置失败: {元件名}")` 提示用户，不保留部分放置结果
