...
## Epic 5+ 可选规划：基于 Microsoft Agent Framework 的会话管理升级

> **状态**：可选后期规划，当需要多轮对话记忆时实施。当前 Epic 4 已通过 EF Core 直写完成历史持久化，本章节记录更完整的框架集成路径备用。

### 框架调研结论（Microsoft.Agents.AI）

分析 `https://github.com/microsoft/agent-framework/tree/main/dotnet` 源码，发现三个核心扩展点：

| 接口 | 命名空间 | 作用 |
|------|----------|------|
| `AgentSession`（抽象基类） | `Microsoft.Agents.AI` | 会话状态容器，含 `StateBag` 键值存储，支持 JSON 序列化/反序列化 |
| `ChatHistoryProvider`（抽象基类） | `Microsoft.Agents.AI` | 对话历史读写钩子：`ProvideChatHistoryAsync()` + `StoreChatHistoryAsync()` |
| `AIContextProvider`（抽象基类） | `Microsoft.Agents.AI` | 调用生命周期钩子：调用前注入上下文（`InvokingAsync`）+ 调用后处理（`InvokedAsync`） |

**`ChatHistoryProvider` 双阶段生命周期：**
```csharp
// 阶段1：调用前，从存储加载历史消息注入 LLM 上下文
protected override ValueTask<IEnumerable<ChatMessage>> ProvideChatHistoryAsync(
    AgentSession session, CancellationToken ct)

// 阶段2：调用后，将新消息持久化到存储
protected override ValueTask StoreChatHistoryAsync(
    AgentSession session, IEnumerable<ChatMessage> messages, CancellationToken ct)
```

官方内置实现：`InMemoryChatHistoryProvider`（内存缓存）、`CosmosChatHistoryProvider`（Cosmos DB）。

### 当前架构的阻碍

框架钩子**仅在** `AIAgent.RunAsync()` / `RunStreamingAsync()` 生命周期内触发。当前架构有三项不兼容：

1. **调用模式不兼容**：`SchematicService.GenerateStreamAsync` 直接调用 OpenAI SSE，不经过框架生命周期，钩子无法触发
2. **历史字段不匹配**：`StoreChatHistoryAsync` 仅存储 `ChatMessage[]`；项目需要 `CircuitJson`、`IsSuccess` 等业务字段
3. **迁移成本**：需将 `CircuitParserAgent` 改造为 `AIAgent` 派生类，重构 `SchematicService` 调用方式

### Epic 5 实施路径

**触发条件**：需要多轮对话记忆（Agent 记住同一 Session 内的历史原理图上下文参与新一轮生成）
...
**第2步**：实现 `PostgresChatHistoryProvider`
```csharp
public class PostgresChatHistoryProvider(AppDbContext db) : ChatHistoryProvider
{
    protected override async ValueTask<IEnumerable<ChatMessage>> ProvideChatHistoryAsync(
        AgentSession session, CancellationToken ct)
    {
        // 按 sessionId 从 schematic_histories 加载历史，转为 ChatMessage[]
        var histories = await db.SchematicHistories
            .Where(h => h.SessionId == Guid.Parse(session.SessionId))
            .OrderBy(h => h.CreatedAt)
            .ToListAsync(ct);
        return histories.SelectMany(h => new[]
        {
            ChatMessage.CreateUserMessage(h.UserInput),
            ChatMessage.CreateAssistantMessage(h.CircuitJson),
        });
    }
    ...
    protected override async ValueTask StoreChatHistoryAsync(
        AgentSession session, IEnumerable<ChatMessage> messages, CancellationToken ct)
    {
        // 注意：业务字段（CircuitJson、IsSuccess）需通过 session.StateBag 传递
        // 当前 Epic 4 的 TrySaveHistoryAsync 仍负责写入完整业务记录
    }
}
```

**第3步**：改造 `CircuitParserAgent` 为 `AIAgent` 派生类
- 注册 `PostgresChatHistoryProvider` 和 `AIContextProvider`
- 历史注入将自动发生于每次 `RunStreamingAsync` 调用前

**第4步**：`SchematicService` 改用框架调用
```csharp
// 替换当前直接调用 OpenAI SDK 的方式
await foreach (var chunk in _agent.RunStreamingAsync(session, userInput, ct))
    yield return SseEvent.Data(chunk);
```

**第5步**（可选）：`AgentSession` 序列化存 Redis，支持跨请求多轮对话

### 影响范围评估

| 文件 | 变更类型 |
|------|---------|
| `Agents/CircuitParserAgent.cs` | 改造为 `AIAgent` 派生类 |
| `Services/SchematicService.cs` | 改用 `RunStreamingAsync` |
| 新建 `Infrastructure/PostgresChatHistoryProvider.cs` | 新增 |
| `Program.cs` | 注册新 Provider |

**预估工作量**：1 个 Sprint（约 3-5 个故事点），需要在 Epic 4 的 Session Schema 基础上进行。
